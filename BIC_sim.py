# -*- coding: utf-8 -*-
"""
Created on Mon Feb  3 14:03:26 2025
...
if __name__ == '__main__':
    ...
    Round = 1
    Epoch = 100
    Min_point = int(Round*Repeat/2)
    
    Q = Q_mat(J,G)
    D_true,L_true,Phi_true,Psi_true,Cov_true = generator(J,G,Q,case)
    ...
    params_list = [(S_list[i],n,J,G_min,G_max,rho_0,rho_sigma,theta,tol,max_iter,Repeat,Round,Min_point,multi) for i in range(Epoch)]
    with Pool(processes= Epoch) as p:
        results = p.starmap(BIC_compare_process, params_list)
        
    for g_alm,g_efa in results:
        ALM_list.append(g_alm)
        EFA_list.append(g_efa)
        
    ALM_list = np.array(ALM_list)
    EFA_list = np.array(EFA_list)
    
    ALM_name = 'BF_minor_sim/BIC/ALM_' + str(J) + '_' +  str(G) + '_' + str(n) + '_' + str(case) 
    np.save(ALM_name,ALM_list)
    EFA_name = 'BF_minor_sim/BIC/EFA_' + str(J) + '_' +  str(G) + '_' + str(n) + '_' + str(case)
    np.save(EFA_name,EFA_list)
    
    
    print(f'The estimated groups by alm are {ALM_list}')
    print(f'The average groups by alm are {np.mean(ALM_list)}')
    print(f'The estimated groups by efa are {EFA_list}')
    print(f'The average groups by efa are {np.mean(EFA_list)}')
