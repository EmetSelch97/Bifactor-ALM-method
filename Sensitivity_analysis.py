# -*- coding: utf-8 -*-
"""
Created on Mon Feb  3 15:00:33 2025
...
def Bf_eval(L,Psi,D,Q,L_true,Psi_true,D_true,Q_true,P_list):
    ...
    L_err_list = np.zeros(len(P_list))
    Psi_err_list = np.zeros(len(P_list))
...
def Sen_process(S,n,J,G,Pair,rho_0,gamma_0,tol,max_iter,Repeat,Round,Min_point,Sigma_set,Theta_set,L_true,Psi_true,D_true,Q_true,P_list,multi=False):
    ...
    for i in range(N1):
        for j in range(N2):
            ...
            L_alm[:,1:] = np.where(np.abs(L_alm[:,1:])>tol,L_alm[:,1:],0)
...
if __name__ == '__main__':
    ...
    Repeat=50
    Round=1
    Min_point = int(Repeat*Round/2)
    multi = False
    
    Sigma_set = np.array([5,10,15])
    Theta_set = np.array([0.25,0.5,0.75])
    ...
    params_list = [(S_list[i],n,J,G,Pair,rho_0,gamma_0,tol,max_iter,Repeat,Round,Min_point,Sigma_set,Theta_set,L_true,Psi_true,D_true,Q_true,P_list,multi) for i in range(Epoch)]
    with Pool(processes= Epoch) as p:
        results = p.starmap(Sen_process, params_list)
    
    iter_num = 0
    for L_err_list,Psi_err_list,D_err_list,EMC_list,ACC_list,Time_list,Compute_num_list in results:
        
        
        L_name = 'BF_minor_sim/Sen/L_err_' + str(J) + '_' +  str(G) + '_' + str(n) + '_' + str(case) + '_' + str(iter_num)
        np.save(L_name,L_err_list)
        
        Psi_name = 'BF_minor_sim/Sen/Psi_err_' + str(J) + '_' +  str(G) + '_' + str(n) + '_' + str(case) + '_' + str(iter_num)
        np.save(Psi_name,Psi_err_list)
        
        D_name = 'BF_minor_sim/Sen/D_err_' + str(J) + '_' +  str(G) + '_' + str(n) + '_' + str(case) + '_' + str(iter_num)
        np.save(D_name,D_err_list)
        
        EMC_name = 'BF_minor_sim/Sen/EMC_' + str(J) + '_' +  str(G) + '_' + str(n) + '_' + str(case) + '_' + str(iter_num)
        np.save(EMC_name,EMC_list)
        
        ACC_name = 'BF_minor_sim/Sen/ACC_' + str(J) + '_' +  str(G) + '_' + str(n) + '_' + str(case) + '_' + str(iter_num)
        np.save(ACC_name,ACC_list)
        
        Time_name = 'BF_minor_sim/Sen/Time_' + str(J) + '_' +  str(G) + '_' + str(n) + '_' + str(case) + '_' + str(iter_num)
        np.save(Time_name,Time_list)
        
        Compute_num_name = 'BF_minor_sim/Sen/Compute_num_' + str(J) + '_' +  str(G) + '_' + str(n) + '_' + str(case) + '_' + str(iter_num)
        np.save(Compute_num_name,Compute_num_list)
